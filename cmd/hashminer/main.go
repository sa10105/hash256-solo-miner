...
func runStatus(args []string) error {
	...
	rpcURLs := configuredRPCURLs(*rpcURL)

	ctx, cancel := context.WithTimeout(context.Background(), 30*time.Second)
	defer cancel()

	client, activeRPC, err := dialHashClient(ctx, rpcURLs, *contract)
	if err != nil {
		return err
	}
	defer client.Close()
...
func runMine(args []string) error {
	...
	for {
		...
		txOpts := hash256.TxOptions{
			GasLimit: *gasLimit,
			TipCap:   tipCap,
			FeeCap:   feeCap,
		}
		txHash, err := client.SubmitMine(ctx, key, new(big.Int).SetUint64(result.Nonce), txOpts)
		if err != nil && len(rpcURLs) > 1 {
			client, activeRPC, txHash, err = submitMineWithFallback(ctx, rpcURLs, activeRPC, *contract, key, result.Nonce, txOpts)
			if err == nil {
				defer client.Close()
			}
		}
...
func configuredRPCURLs(flagValue string) []string {
	var urls []string
	if strings.TrimSpace(flagValue) != "" {
		urls = append(urls, splitRPCURLs(flagValue)...)
	} else {
		urls = append(urls, splitRPCURLs(env("ETH_RPC_URL", ""))...)
	}
	urls = append(urls, splitRPCURLs(env("ETH_RPC_FALLBACK_URLS", ""))...)

	seen := map[string]bool{}
	out := make([]string, 0, len(urls))
	for _, rpcURL := range urls {
		rpcURL = strings.TrimSpace(rpcURL)
		if rpcURL == "" || seen[rpcURL] {
			continue
		}
		seen[rpcURL] = true
...
func splitRPCURLs(value string) []string {
	return strings.FieldsFunc(value, func(r rune) bool {
		return r == ',' || r == ';' || r == '\n' || r == '\r' || r == '\t' || r == ' '
	})
}

func dialHashClient(ctx context.Context, rpcURLs []string, contract string) (*hash256.Client, string, error) {
	if len(rpcURLs) == 0 {
		return nil, "", fmt.Errorf("Ethereum RPC URL is required; set ETH_RPC_URL")
	}
	...
	return nil, "", fmt.Errorf("all Ethereum RPC URLs failed: %s", strings.Join(failures, "; "))
}

func submitMineWithFallback(ctx context.Context, rpcURLs []string, failedRPC string, contract string, key *ecdsa.PrivateKey, nonce uint64, opts hash256.TxOptions) (*hash256.Client, string, common.Hash, error) {
	var failures []string
	for _, rpcURL := range rpcURLs {
		if rpcURL == failedRPC {
			continue
		...
		txHash, err := client.SubmitMine(ctx, key, new(big.Int).SetUint64(nonce), opts)
		if err == nil {
			fmt.Printf("rpc:       switched to %s\n", rpcURL)
			return client, rpcURL, txHash, nil
		}
		client.Close()
		failures = append(failures, fmt.Sprintf("%s: %v", rpcURL, err))
	}
	return nil, "", common.Hash{}, fmt.Errorf("submit failed on primary RPC %s and all fallback RPCs failed: %s", failedRPC, strings.Join(failures, "; "))
}
...
環境變數，可放在專案根目錄 .env:
  ...
  ETH_RPC_FALLBACK_URLS   comma/space separated fallback RPC URLs
